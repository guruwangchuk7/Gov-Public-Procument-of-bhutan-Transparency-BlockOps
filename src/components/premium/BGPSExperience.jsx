...
export function DashboardExperience({ role }) {
...
export function PublicPortal({ embedded = false, notify }) {
  const [query, setQuery] = useState("");
...
export function TenderDetailPage({ id }) {
...
function AgencyDashboard({ activeTab, query, setQuery, filter, setFilter, tenders: list, openModal, notify }) {
...
function SupplierDashboard({ activeTab, query, setQuery, tenders: list, openModal, notify }) {
  return (
    <>
...
function AuditorDashboard({ notify }) {
...
function MiniChartCard({ title, value, line = false }) {
...
function PublicNav({ embedded = false }) {
  return (
...
function Topbar({ role, onMenu, onDemo }) {
  return (
...
function HeroStrip({ role, onPrimary, onDemo }) {
...
function VisualTabs({ tabs, active, onChange }) {
  return (
...
function CreateTenderModal({ onClose, onToast }) {
...
function SubmitBidModal({ onClose, onToast }) {
  const [hash, setHash] = useState("");
...
function WinnerModal({ onClose, onToast }) {
  return (
...
function TransactionModal({ title, onClose, onToast }) {
  const [step, setStep] = useState(0);
...
function TenderDrawer({ tender, role, onClose, onBid }) {
  return (
...
function Filters({ query, setQuery, filter, setFilter, options }) {
...
function TenderRow({ tender, onDetail, onAction }) {
  return (
...
function ProgressStep({ step, active }) {
...
function BeforeAfter({ title, items, tone }) {
...
function MockUpload({ label = "Upload document" }) {
...
function PillLink({ href, children, icon: Icon, variant = "blue" }) {
