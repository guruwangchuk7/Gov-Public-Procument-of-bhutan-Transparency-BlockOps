...
import { ShieldCheck, Cpu, ArrowRight, Loader2, Link as LinkIcon, Building2, Landmark } from 'lucide-react';
...
export default function BlockchainVerificationQueue() {
  ...
  const fetchPendingEvents = async () => {
    const { data, error } = await supabase
      ...
      .select('*, agencies(agency_name), suppliers(company_name)')
  ...
  const handleConfirm = async (event) => {
    ...
    try {
      ...
      if (event.related_agency_id) payload.agencyId = event.related_agency_id;
      if (event.related_supplier_id) payload.supplierId = event.related_supplier_id;
        ...
        {pendingEvents.map((event) => (
          <div key={event.id} className="p-4 bg-slate-800/50 rounded-2xl border border-slate-800 flex items-center justify-between group">
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center ${event.related_agency_id ? 'text-primary' : 'text-emerald-500'}`}>
                {event.related_agency_id ? <Landmark size={20} /> : <Building2 size={20} />}
              </div>
              <div>
